...
export function SupportForm() {
  ...
  async function onSubmit(values: SupportTicketInput) {
    ...
    if (!response.ok) {
      setMessage({ type: "error", text: data.message ?? "حدث خطأ أثناء إرسال طلب الدعم. حاول مرة أخرى." });
      return;
    }
  ...
  return (
    <form onSubmit={handleSubmit(onSubmit)} className="grid gap-5 rounded-lg border border-slate-200 bg-white p-5 shadow-sm md:p-8">
      ...
      <div className="grid gap-5 md:grid-cols-2">
        ...
        <Field label="موضوع الرسالة" error={errors.subject?.message}>
          <input className="input-field" {...register("subject")} />
        </Field>
      </div>

      <Field label="تفاصيل المشكلة أو الاستفسار" error={errors.message?.message}>
        <textarea className="input-field min-h-32" {...register("message")} />
      </Field>
